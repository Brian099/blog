...
async function handleEditorAttachmentUpload(e) {
    ...
    try {
        ...
        if (data.success && window.ueEditor) {
            ...
            const iconName = extMap[ext] || 'text';
            const linkHtml = `<p><img class="filetype-icon" src="/users/filetype/${iconName}.svg" alt="${ext.toUpperCase()}" style="width:18px;height:18px;vertical-align:-3px;margin-right:5px;border:none;display:inline-block;" /> <a href="${data.url}" title="${fileName}">${fileName}</a></p>`;
